...
import { getErrorMessage, useLogout, useRegister } from '../service/authService.js';
import { useForm} from 'react-hook-form';

export default function register() {
    const navigate = useNavigate();
    const EMAIL_REGEX = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i;
    const signup = useRegister(navigate);
    const { register, handleSubmit, formState } = useForm();
    const { errors } = formState;

    const backendErrorMessage = signup.isError ? getErrorMessage(signup.error) : null;
    const successMessage = signup.isSuccess ? 'Đăng nhập tài khoản thành công! Đang chuyển hướng sang Home' : null;
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 font-sans">
            <div className="w-full max-w-md bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-lg p-6">
                ...
                <p className="text-white/70 mb-6">Chào mừng người mới</p>

                {/* Success Message from signup or signup Success */}
                {(successMessage) && (
                    <div className="mb-4 text-sm text-green-300 bg-green-900/30 p-3 rounded-lg border border-green-700/50">
                        {successMessage || successMessageFromsignup}
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit(data => signup.mutate(data))}>
                    ...
                    <button
                        ...
                        disabled={signup.isPending} // Dùng isPending từ useMutation
                    >
                        {signup.isPending && (
                ...
                <div className="mt-6 text-sm text-white/70 text-center">
                    Bạn đã có tài khoản? <Link to="/login" className="text-sky-300 hover:underline font-medium">Đăng nhập</Link>
                </div>
            </div>
        </div>
    );
